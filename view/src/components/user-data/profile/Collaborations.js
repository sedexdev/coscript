        ...
        <Fragment>
            {displayCollaborateDeleteWindow && (
                <div className='delete-project-window'>
                    <p className='delete-project-heading'>Leave this project?</p>
                    <form className='edit-project-form' onSubmit={(e) => onDeleteCollaborate(e)}>
                        <div className='project-edit-btn-container'>
                            {savingCollaborate && <img src={spinner} className='update-project-loading-spinner' alt='Loading spinner' />}
                            <button className='cancel-btn' onClick={removeCollaborateDeleteWindow}>
                                Cancel
                            </button>
                    ...
                    {user && userCollaborations && userCollaborations.length ? (
                        <div className='my-projects-list'>
                            {userCollaborations.map((project) => {
                                return (
                                    <div className='profile-collaborations-list-element-wrapper' key={project.projectId}>
                                        <li
                                            className='profile-project-list-element'
                                            onClick={async () => {
                                                await loadProject(project);
                                                history.push(`/editor${project.url}`);
                                            }}>
                                            {project.title}
                                        </li>
                                        <div className='project-options-container'>
                                            <button className='delete-project-btn' onClick={openCollaborateDeleteWindow}>
                        ...
                        <div className='projects-warning'>
                            <p>
                                You aren't collaborating on any projects right now. Find one{" "}
                                <Link to='/projects'>
